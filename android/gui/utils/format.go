...
// CleanDisplayText formats raw text for clean UI rendering in Gio on Android.
...
// control characters, and removes invisible zero-width unicode characters.
func CleanDisplayText(raw string) string {
	...
	for _, r := range s {
		...
		case r == '\v' || r == '\f':
			b.WriteRune('\n')
		case r < 32:
			// Strip ASCII control characters (null, bell, escape, etc.)
			continue
		case r == 0x7F:
			// ASCII DEL
			continue
		...
		case unicode.IsControl(r):
			// Strip other unicode control characters
			continue
		default:
			b.WriteRune(r)
		}
